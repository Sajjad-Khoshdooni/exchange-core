...
def mexc_send_sign_request(http_method: str, url_path: str, payload: dict):
    ...
    query_string = urlencode(data, True)

    header = {'X-MEXC-APIKEY': API_KEY, 'Content-Type': 'application/json'}
    if query_string:
        query_string = '{}&recvWindow=5000&timestamp={}'.format(query_string, get_time_stamp())
    else:
        query_string = 'recvWindow=5000&timestamp={}'.format(get_time_stamp())

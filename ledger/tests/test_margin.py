...
from ledger.models import Asset, Wallet, OTCRequest, OTCTrade, Trx, CloseRequest
...
class MarginTestCase(TestCase):
    ...
    def test_transfer(self):
        ...
        self.assertEqual(spot_wallet.balance, 0)
    ...
    def test_loan(self):
        ...
        self.assert_margin_info(TO_TRANSFER_USDT, 3 * TO_TRANSFER_USDT, 4 * TO_TRANSFER_USDT, Decimal('1.333'))
        ...
        self.loan('ADA', 1, type='repay')
        self.assert_margin_info(TO_TRANSFER_USDT, 2 * TO_TRANSFER_USDT - 1 * XRP_USDT_PRICE, 3 * TO_TRANSFER_USDT - 1 * XRP_USDT_PRICE, Decimal('1.505'))
    ...
    def test_valid_short_sell(self):
        self.pass_quiz()
        self.transfer_usdt(TO_TRANSFER_USDT)

        loan_amount = TO_TRANSFER_USDT / XRP_USDT_PRICE
        self.loan(self.xrp.symbol, loan_amount)

        self.assertEqual(check_margin_level(), 0)

        self.assert_margin_info(TO_TRANSFER_USDT, TO_TRANSFER_USDT, 2 * TO_TRANSFER_USDT, Decimal('2'))
        ...
        otc_request = OTCRequest.new_trade(
            ...
            from_asset=self.usdt,
            to_asset=self.xrp,
            from_amount=Decimal(loan_amount * XRP_USDT_PRICE),
            allow_dust=True
        )
        ...
        print('///////////////CLOSE_REQUEST///////////////')
        CloseRequest.close_margin(self.account, CloseRequest.USER)
        self.print_wallets()
        self.assertEqual(check_margin_level(), 0)

    def test_invalid_short_sell(self):
        self.pass_quiz()
        self.transfer_usdt(TO_TRANSFER_USDT)
        ...
        try:
            ...
            self.assertEqual(type(e), InsufficientBalance)

        self.assertEqual(check_margin_level(), 0)

    def test_short_sell_liquidation(self):
        self.pass_quiz()
        self.transfer_usdt(TO_TRANSFER_USDT)
        ...
        set_price(self.xrp, 3 * XRP_USDT_PRICE)
        check_margin_level()
        self.print_wallets(self.account)
        self.assertFalse(Wallet.objects.filter(~Q(balance=Decimal('0')), account=self.account, market__in=['margin', 'loan']).exists())
        self.assertTrue(Trx.objects.filter(created__gte=timestamp, receiver__account=MARGIN_INSURANCE_ACCOUNT, sender__account=self.account).exists())

    def test_short_sell_liquidation2(self):
        self.pass_quiz()
        self.transfer_usdt(TO_TRANSFER_USDT)
        ...
        otc_request = OTCRequest.new_trade(
            ...
            from_asset=self.usdt,
            to_asset=self.xrp,
            from_amount=Decimal(TO_TRANSFER_USDT / 2),
            allow_dust=True
        )
        OTCTrade.execute_trade(otc_request)
        check_margin_level()
